...
int main(int argc, char** argv)
{
    ...
    std::mutex add_mutex, del_mutex;
    {
        ...
        for(uint64_t i=imported_edges;i<raw_edges_len;i++)
        {
            {
                const auto &e = raw_edges[i];
                if(graph.need_update_tree_add(update_func, labels, {e.first, e.second}, false))
                {
...

            {
                ...
                if(graph.need_update_tree_del(labels, {e.first, e.second}, false))
                {
                    atomic_append(del_edge, del_edge_len, {e.first, e.second}, del_mutex);
                }
                else
                {
                    graph.del_edge({e.first, e.second}, false);
                }
            }
        };
        auto end = std::chrono::system_clock::now();
