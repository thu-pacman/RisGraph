...
int main(int argc, char** argv)
{
    ...
    Graph<void> graph(num_vertices, raw_edges_len, false, true, false);
    ...
    std::mutex add_mutex, del_mutex;
    {
        ...
        for(uint64_t i=imported_edges;i<raw_edges_len;i++)
        {
            {
                const auto &e = raw_edges[i];
                if(graph.need_update_tree_add(update_func, labels, {e.first, e.second}, true))
                {
                    atomic_append(add_edge, add_edge_len, {e.first, e.second}, add_mutex);
                }
                else
                {
                    graph.add_edge({e.first, e.second}, true);
...

            {
                ...
                if(graph.need_update_tree_del(labels, {e.first, e.second}, true))
                {
                    atomic_append(del_edge, del_edge_len, {e.first, e.second}, del_mutex);
                }
                else
                {
