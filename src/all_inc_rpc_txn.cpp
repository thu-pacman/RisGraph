...
int main(int argc, char** argv)
{
    ...
    struct Update
    {
        ...
        decltype(graph)::edge_type edges[16];
    ...
    std::atomic_uint64_t txns_len(0), batch_size(0);
    std::vector<Update> txns;
    std::mutex txns_mutex;
    std::atomic_bool txns_setted = false;
    std::chrono::high_resolution_clock::time_point txns_first;
    ...
    {
        uint64_t num_changes = txns_len.load();
        auto min_time = time;
        if(txns_setted.load(std::memory_order_acquire)) min_time = std::min(min_time, txns_first);
        return (uint64_t)std::chrono::duration_cast<std::chrono::milliseconds>(time-min_time).count() > 0.5*timeout_ms || num_changes > num_changes_threshold;
...

    {
        ...
        #pragma omp parallel
        {
            ...
            for(uint64_t fid=client_begin;fid<client_begin+num_clients;fid++)
            {
                ...
                {
                    ClientTxnUpdateRequest request;
                    Update update;
                    bool is_end = false;
                    while(!is_end)
                    {
                        ...
                        update.time = request.request_time;
                        for(update.num_updates = 0;update.num_updates<request.num_updates;update.num_updates++)
                        {
                            switch(request.types[update.num_updates])
                            {
                                case ClientTxnUpdateRequest::Type::Add:
                                {
#if defined(BFS) || defined(WCC)
                                    decltype(graph)::edge_type edge = {request.srcs[update.num_updates], request.dsts[update.num_updates]};
#elif defined(SSSP) || defined(SSWP)
                                    decltype(graph)::edge_type edge = {request.srcs[update.num_updates], request.dsts[update.num_updates], request.datas[update.num_updates]};
#endif
                                    update.types[update.num_updates] = Update::Type::Add;
                                    update.edges[update.num_updates] = edge;
                                    break;
                                }
                                case ClientTxnUpdateRequest::Type::Del:
                                {
#if defined(BFS) || defined(WCC)
                                    decltype(graph)::edge_type edge = {request.srcs[update.num_updates], request.dsts[update.num_updates]};
#elif defined(SSSP) || defined(SSWP)
                                    decltype(graph)::edge_type edge = {request.srcs[update.num_updates], request.dsts[update.num_updates], request.datas[update.num_updates]};
#endif
                                    update.types[update.num_updates] = Update::Type::Del;
                                    update.edges[update.num_updates] = edge;
                                    break;
                                }
                                case ClientTxnUpdateRequest::Type::End:
                                {
                                    is_end = true;
                                    update.types[update.num_updates] = Update::Type::End;
                                    break;
                                }
                            }

                        }
                        requests[lfid]->push(update);
                    }
                });
                fibers.emplace_back([&, fid, lfid=fid-client_begin]()
                {
                    ...
                    while(!is_end)
                    {
                        ...
                        for(uint32_t i=0;i<update.num_updates;i++)
                        {
                            if(update.types[i] != Update::Type::End)
                            {
                                WAL log = {update.types[i] == Update::Type::Add ? WAL::Type::Add : WAL::Type::Del, 
                                           (uint32_t)fid, update.edges[i].src, update.edges[i].dst, update.edges[i].data};
                        ...
                        for(uint32_t i=0;i<update.num_updates;i++)
                        {
                            switch(update.types[i])
                            {
                                case Update::Type::Add:
                                {
                                    is_safe = is_safe && (!graph.need_update_tree_add(update_func, labels, update.edges[i], directed));
                                    break;
                                }
                                case Update::Type::Del:
                                {
                                    is_safe = is_safe && (!graph.need_update_tree_del(labels, update.edges[i], directed));
                                    break;
                                }
                        ...
                        {
                            local_batch_size ++;
                            auto local_version_id = ++version_id;
                            for(uint32_t i=0;i<update.num_updates;i++)
                            {
                                switch(update.types[i])
                                {
                                    case Update::Type::Add:
                                    {
                                        response.version_ids[i] = local_version_id;
                                        graph.add_edge(update.edges[i], directed);
#ifdef ENABLE_KVS
                                    ...
                                    {
                                        response.version_ids[i] = local_version_id;
                                        graph.del_edge(update.edges[i], directed);
#ifdef ENABLE_KVS
                        ...
                        {
                            local_batch_size ++;
                            for(uint32_t i=0;i<update.num_updates;i++) update.version_ids[i] = &response.version_ids[i];
                            std::unique_lock<boost::fibers::mutex> lock(mutex);
                            auto idx = atomic_append(txns, txns_len, update, txns_mutex);
                            if(idx == 0)
                            {
                                txns_first = update.time;
                                txns_setted.store(true, std::memory_order_release);
                            }
                            left_clients--;
            ...
            while(!loop_end)
            {
                ...
                if(need_break)
                {
                    ...
                    if(tid == 0)
                    {
                        ...
                        for(uint64_t i=0;i<txns_len.load();i++)
                        {
                            graph.clear_modified();
                            for(uint32_t j=0;j<txns[i].num_updates;j++)
                            {
                                *txns[i].version_ids[j] = ++version_id;
                                const auto &e = txns[i].edges[j];
                                if(txns[i].types[j] == Update::Type::Add)
                                {
#ifdef ENABLE_KVS
                        ...
                        if(num_loops % 100 == 0)
                        {
                            ...
                            fprintf(stderr, "unsafe_txns: %6lu, ", txns_len.load());
