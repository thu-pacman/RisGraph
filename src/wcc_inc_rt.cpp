...
int main(int argc, char** argv)
{
    if(argc <= 5)
    {
        fprintf(stderr, "usage: %s graph imported_rate timeout_ms percent clients\n", argv[0]);
        exit(1);
    }
                    ...
                    while(num_clients && (!add_end || !del_end))
                    {
                        ...
                        if(is_add)
                        {
                            ...
                            local_batch_size += 1;
                            {
                                ...
                                if(graph.need_update_tree_add(update_func, labels, edge, false))
                                {
                        ...
                        else
                        {
                            ...
                            {
                                decltype(graph)::edge_type edge = {raw_edges[i].first, raw_edges[i].second};
                                if(graph.need_update_tree_del(labels, edge, false))
                                {
