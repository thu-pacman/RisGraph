...
int main(int argc, char** argv)
{
    ...
    if(argc > 8)
    {
        std::tie(add_raw_edges, add_raw_edges_len) = mmap_binary(argv[7]);
        std::tie(del_raw_edges, del_raw_edges_len) = mmap_binary(argv[8]);
    }
    else
    {
...

    {
        ...
        #pragma omp parallel
        {
                ...
                fibers.emplace_back([&, fid, lfid=fid-client_begin]()
                {
                    ...
                    for(bool is_add = true; add < add_raw_edges_len || del < del_raw_edges_len; is_add = !is_add)
                    {
                        if(is_add)
                        {
                            ...
                            request = {(uint32_t)fid, ClientUpdateRequest::Type::Add, e.first, e.second, (e.first+e.second)%128, std::chrono::high_resolution_clock::now()};
                            add+=clients;
                        }
                        else
                        {
                            ...
                            request = {(uint32_t)fid, ClientUpdateRequest::Type::Del, e.first, e.second, (e.first+e.second)%128, std::chrono::high_resolution_clock::now()};
                            del+=clients;
                        }
            ...
            while(num_clients && num_updates.load() < add_raw_edges_len+del_raw_edges_len)
            {
